<?php  
	session_start();
	include "connect.php";
	include "header.php";
    $username = $_SESSION['username'];
    $id_user = $_SESSION['id'];
    if (!$username) {
        session_destroy();
        echo'<script type="text/javascript">alert("Has not Login Yet !");</script>';
        echo'<script>location.href="login.php"</script>';
    }
    else{
    	include "navbarmain.php";

		$quer = "SELECT b.id,b.judul_buku,b.lokasi_buku,b.no_buku,b.publisher,b.author,b.tahun_terbit FROM buku b LEFT JOIN user_favorit uf ON b.id = uf.id_buku WHERE uf.id_user = '$id_user'";
		$select_fav = mysqli_query($con,$quer);

		$count=0;
		while ($printBook = mysqli_fetch_assoc($select_fav)) {
			if ($printBook['id']) {
				$id[$count] = $printBook['id'];
				$judul_buku[$count] = $printBook['judul_buku'];
				$publisher[$count] = $printBook['publisher'];
				$author[$count] = $printBook['author'];
				$yearDate[$count] = $printBook['tahun_terbit'];
				$isbn[$count] = $printBook['no_buku'];
				$link[$count] = $printBook['lokasi_buku'];
				$count++;
			}
		}
    }
?>

<html>
<head>
	<title></title>
	<style>
		.book-list{
			margin: 0 3em 0 3em;
			padding: 20px 15px 20px 15px;
			position: relative;
			display: block;
		}
		table {
			width: 100%; 
			height: 100%;
		}
		h3{
			display: inline-block;
			padding-top: 0.625em;
			position: relative;
			margin-right: 0.5em;
		}
		tbody{
			border: 1px;
			border-style: none;
			border-bottom-style: inset; 
		}

		body {
		    padding: 0;
		    margin: 0;
		    height: 100vh;
		    width: 100%;
		    background: #FFFFFF;
		}

	</style>
</head>
<body>
	 <div class="book-list">
	 	<h4 style="text-transform: uppercase;"><?php echo $username ?>'s Favorite Book(s)</h4>
		<table id="book-table">
			<?php  
				for ($index=0; $index < $count ; $index++) { 
					echo'
						<tbody>
							<td style="vertical-align: top;">
								<h3 style="color:blue; cursor:default;">
									'.$judul_buku[$index].'
								</h3>
								<div title="publisher" class="publisher">'.$publisher[$index].'</div>
								<div class="author">'.$author[$index].'
								</div>
							</td>
							<td style="float: right; padding-top:1em;">
                                <div class="tags-container">
                                <div class="bookDetailsBox" style="">
	                				<div class="bookProperty property_year" dir="rtl">
	                    				<span>Year:</span>
	                    				'.$yearDate[$index].'
	                				</div>
					                <div class="bookProperty property_isbn" dir="rtl">
					                    <span id="isbn-'.$index.'">Book Id:</span>
					                    '.$isbn[$index].'
					                </div>
					           	</div>
					             	<a target = "_blank" href="'.$link[$index].'" download="'.$judul_buku[$index].'"><button type="button" id ="btn-download-'.$index.'" onclick=" updateDownload('.$isbn[$index].')"class="btn btn-outline-success" ><i class="fas fa-download" style="color:gray;"></i>Download</button></a>
					             	<a target = "_blank" href="readPDF.php?id='.$id[$index].'" onclick=" updateRead('.$isbn[$index].')"><button type="button" id ="btn-read-'.$index.'" class="btn btn-outline-primary"><i class="fas fa-book-open" style="color:gray;"></i>Read</button></a>
					             	<button type="button" class="btn btn-outline-danger" onclick="removeFavorite('.$id[$index].','.$_SESSION['id'].')"><i class="fas fa-heart" style="color:red;"></i>Unfavorite</button>
				            	</div>
                    		</td>
						</tbody>
					';
				}
			?>
		</table>
	 </div>
</body>
</html>

<script>
	function updateDownload(data){
		var isbn = data;
		$.ajax({
			url:'downloadcount.php',
			type:'POST',
			data:{
				isbn:isbn
			},
			success:function(response){
				if (response == 1){
					alert("Download Count+=1");
				}
				else
					alert("Download Count blm bertambah");
			}
		});
	}

	function updateRead(data){
		var isbn = data;
		$.ajax({
			url:'readcount.php',
			type:'POST',
			data:{
				isbn:isbn
			},
			success:function(response){
				if (response == 1){
					alert("Read Count+=1");
				}
				else
					alert("Read Count blm bertambah");
			}
		});
	}

	function removeFavorite(id_buku,id_user){
		$.ajax({
			url:'removeFavBook.php',
			type:'POST',
			data:{
				id_buku:id_buku,
				id_user:id_user
			},
			success:function(response){
				if (response == 1){
					alert("Unfavorite Book Success");
					location.reload();
				}
				else{
					alert("Can't Unfavorite Book");
				}
			}
		});
	}
</script>